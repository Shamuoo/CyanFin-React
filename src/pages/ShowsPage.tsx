...
export default function ShowsPage() {
  ...
  const { data, isFetching } = useQuery({ queryKey: ['shows', sort, order, genre, start], queryFn: () => api.shows({ sort, order, genre, start }) })
  const { data: genres = [] } = useQuery({ queryKey: ['genres-series'], queryFn: () => api.genres('Series') })

  useEffect(() => { if (!data) return; setTotal(data.total); setAllItems(prev => start === 0 ? data.items : [...prev, ...data.items]) }, [data, start])
    ...
    <div ref={scrollRef} className="h-full overflow-y-auto scrollbar-hide" style={{ background: 'var(--bg)' }}>
      <div style={{ padding: '24px var(--pad) 12px' }}>
        ...
        <div className="flex gap-2 flex-wrap">
          <select value={sort} onChange={e => setSort(e.target.value)} className={sel} style={ss}>{[['SortName','A–Z'],['PremiereDate','Year'],['CommunityRating','Rating'],['DateCreated','Date Added']].map(([v,l]) => <option key={v} value={v}>{l}</option>)}</select>
          <select value={order} onChange={e => setOrder(e.target.value)} className={sel} style={ss}><option value="Ascending">↑ Asc</option><option value="Descending">↓ Desc</option></select>
          <select value={genre} onChange={e => setGenre(e.target.value)} className={sel} style={ss}><option value="">All Genres</option>{genres.map(g => <option key={g} value={g}>{g}</option>)}</select>
        </div>
      </div>
