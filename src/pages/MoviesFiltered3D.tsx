...
export default function MoviesFiltered3D() {
  const { setDetailItemId } = useStore()
  const { data, isLoading } = useQuery({
    queryKey: ['movies-3d'],
    queryFn: () => api.movies3d(),
    staleTime: 10 * 60_000,
  })
  ...
  return (
    <div className="h-full overflow-y-auto scrollbar-hide" style={{ background: 'var(--bg)', padding: '24px var(--pad) 48px' }}>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl tracking-[0.4em] uppercase" style={{ fontFamily: 'var(--font-display)', color: 'var(--cream)', opacity: 0.5 }}>3D Movies</h1>
          {total > 0 && <p className="text-[9px] mt-0.5" style={{ color: 'var(--muted)', opacity: 0.3 }}>{total} 3D films in library</p>}
        </div>
      </div>
      {isLoading
        ...
        : items.length === 0
          ? <p className="text-sm py-16 text-center" style={{ color: 'var(--muted)', opacity: 0.25 }}>No 3D movies found in library.<br/>Jellyfin needs 3D metadata tagged on the video streams.</p>
